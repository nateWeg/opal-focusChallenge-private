...
| **Welcome emails** | `updater/email_notify.py` | Sends a personal welcome from `nate@opal.so` (Gmail SMTP) when a new member first appears. Auto-runs inside `build_leaderboard.py`. **Dry-run unless `SEND_EMAILS=1`.** Needs `GMAIL_USER` + `GMAIL_APP_PASSWORD` in `.env`. |
